...
void Ophido::draw(GLuint  ModelView, GLuint vPosition, GLuint vColor, float rotateX, float rotateY, float rotateZ) {
	vec3 origin(0,0,0);
	for (int i = 0; i < length; i++) {
		body.at(i).draw(ModelView, vPosition, vColor, origin, rotateX, rotateY, rotateZ);
	}
}
