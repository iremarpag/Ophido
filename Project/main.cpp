...
void ophidoPowerupInteraction() {
	...
	if (isOverlapping(o1, o2, s1, s2)) {
		...
		switch (powerup.type) {
			case 0://wall pass
				powerupCounter = 500; //10 sec
				powerupEffect = 250;//5 sec
				break;
			case 1://immortal
				powerupCounter = 1000; //20 sec
				powerupEffect = 250;//5 sec
				break;
			case 2://double
				powerupCounter = 2000; //40 sec; 
				int l = ophido.length;
				for (int i = 0; i < l; i++) {
					ophido.grow();
				}
...
void timer(int p) {
	...
	if (powerupEffect && powerup.type == 0) { // wall pass mode activated no collusion
	}
	else {
		if (ophido.body.at(0).x + ophido.body.at(0).size > 1 || ophido.body.at(0).x - ophido.body.at(0).size < -1 ||
			ophido.body.at(0).y + ophido.body.at(0).size > 1 || ophido.body.at(0).y - ophido.body.at(0).size < -1) {
			printf("Wall collision GAME OVER!!\n");
			end = 1;
		}
	}
	...
	if (!end) {
		...
		if (enemyVis) {
			enemy.tick();
			if (powerupEffect && powerup.type == 1) { // immortal mode activated no interaction
			}
			else {
...
void reshape(int w, int h) {
	...
	mat4  projection = Perspective(45.0, aspect, 0.5, 6.5);
...
void display(void) {
	...
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	wall.draw(ModelView, vPosition, vColor, vNormal, vTexCoord, LightPos, light_position, viewer_pos, Theta[Xaxis], Theta[Yaxis], Theta[Zaxis]);
	DrawingModeMenu(drawing_mode);
	glutSwapBuffers();
}
